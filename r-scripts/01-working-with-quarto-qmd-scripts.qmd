...
editor: 
  markdown: 
    wrap: 72
editor_options: 
  chunk_output_type: console #-> makes sure that output is always shown in console
---
...
e.g. compared to html (how most internet pages are written), which
allows much more formatting, and makes files practically not readable
anymore with a regular text editor.
...
#Line below opens the package 'tidyverse' from the complete library
library(tidyverse)
...
print(data1)
# plot as a histogram (::take the function ggplot from the package ggplot2)
...
|------------------|------------------------------------------------------|
| eval            | Evaluate the code chunk (if false, just echos the code into the output). Echo includes the source code in output.                                                                           |
